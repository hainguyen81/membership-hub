...
## 9. QUY TẮC BẢO MẬT DOANH NGHIỆP & BIỆN PHÁP CHỐNG TẤN CÔNG TIÊM NHẬP (INJECTION)

### 9.1. Biện Pháp Chống Tấn Công Tiêm Nhập SQL (SQL Injection - SQLi) Tuyệt Đối
- **Tham Số Hóa Ranh Giới:** Nghiêm cấm hoàn toàn hành vi cộng chuỗi SQL thô, sử dụng định dạng chuỗi động (`String.format()`), hoặc nội suy chuỗi ký tự trực tiếp vào trong câu truy vấn Panache/Hibernate.
- **Ràng Buộc Tham Số Reactive:** Mọi thao tác truy vấn dữ liệu phải ràng buộc tham số một cách tường minh thông qua các thuộc tính được đặt tên (Named Parameters) hoặc theo chỉ mục định vị sẵn của Panache (Ví dụ: `repository.find("tenantId = ?1 AND email = ?2", tenantId, email)`).
- **Bảo Vệ Sắp Xếp Động:** Các tham số sắp xếp động (Dynamic Sorting) truyền lên từ phía người dùng bắt buộc phải được đối chiếu và kiểm tra tính hợp lệ dựa trên một danh sách trắng (Whitelist) gồm tên các trường thực thể được phép. Nghiêm cấm tiêm trực tiếp chuỗi văn bản chưa được xác thực vào mệnh đề `Panache.sort()` hoặc `ORDER BY`.

### 9.2. Phòng Chống Tấn Công Kịch Bản Khách (XSS) & Chính Sách Bảo MẬT Nội Dung (CSP)
- **Tự Động Tránh Ký Tự Khách:** Mọi hoạt động ràng buộc dữ liệu (Data Binding) ở phía Client bên trong môi trường Next.js bắt buộc phải dựa vào cơ chế tự động thoát ký tự đặc biệt (Auto-escaping) mặc định của công cụ React JSX.
- **Hạn Chế Kết Xuất Nguy Hiểm:** Tuyệt đối cấm sử dụng thuộc tính `dangerouslySetInnerHTML` của Next.js trên toàn bộ các giao diện ứng dụng, trừ trường hợp dữ liệu đầu vào chứng minh được đã đi qua một bộ lọc làm sạch HTML (HTML Sanitizer Whitelist) được backend ký số xác thực.
- **Cấu Hình Header CSP Nghiêm Ngặt:** Hệ thống Edge Middleware phải tiêm các tiêu đề bảo mật HTTP không thể thương lượng vào mọi phản hồi định tuyến. Bao gồm: `Content-Security-Policy` (bắt buộc giới hạn thuộc tính script-src, cấm thực thi `unsafe-inline`), `X-Frame-Options: DENY` (chống tấn công Clickjacking), và `X-Content-Type-Options: nosniff`.

### 9.3. Hàng Rào Bảo Mật Chia Sẻ Tài Nguyên Giữa Các Nguồn (CORS) Đa Thuê Bao
- **Cấm Sử Dụng Ký Tự Đại Diện (Wildcard):** Tuyệt đối cấm sử dụng ký tự đại diện toàn cục (`Access-Control-Allow-Origin: *`) trong các cấu hình môi trường chạy thực tế (Production).
- **Xác Thực Nguồn Gốc Theo Thuê Bao:** Bộ lọc chặn CORS (CORS Interceptor Filter) bên trong Quarkus phải đọc và đối chiếu danh sách các tên miền (Domain Origins) được cấp phép, liên kết trực tiếp với hồ sơ cấu hình đang hoạt động của từng trung tâm trong bảng `sys_tenants`.
- **An Toàn Cho Thông Tin Xác Thực (Credentials) & Preflight:** Các phản hồi xác thực chéo nguồn chỉ được phép thiết lập `Access-Control-Allow-Credentials: true` khi và chỉ khi nguồn gửi yêu cầu (Inbound Origin) trùng khớp hoàn toàn với tên miền đã được đăng ký và xác minh của trung tâm đó.

### 9.4. Hệ Thống Làm Sạch Nhật Ký & Che Dấu Dữ Liệu Định Danh Cá Nhân (PII)
- **Bắt Buộc Làm Sạch Nhật Ký:** Thông tin định danh cá nhân nhạy cảm (PII) bao gồm: mật khẩu dạng văn bản thô, token xác thực từ Firebase/Google/Facebook, mã khóa bí mật của JWT, số thẻ thành viên, và mã băm kết nối Zalo API tuyệt đối không được phép rò rỉ ra console hệ thống hoặc các đường ống lưu trữ log tập trung.
- **Bộ Chặn Che Dấu Tự Động:** Tất cả các bộ điều khiển DTO nhận dữ liệu đầu vào (Web Resource) và các cổng tích hợp đầu ra (ZaloClient, FCMClient) phải sử dụng bộ chặn ghi log (Logging Interceptors) hoặc cấu hình tuần tự hóa Jackson chuyên biệt (`@JsonSerialize`) để tự động áp dụng bộ lọc che dấu (Ví dụ: thay thế chuỗi mật khẩu thô thành `[MASKED_SECURITY_CREDENTIAL]`).
- **Che Dấu Vết Lỗi Hệ Thống:** Nhật ký ngoại lệ (Exception logs) do `Reviewer & Fixer Agent` ghi nhận khi xảy ra lỗi nội bộ không được in kèm giá trị của các biến động tại thời điểm chạy để tránh việc bị khai thác thông tin cấu trúc ngược từ stack trace.

### 9.5. Ranh Giới Cô Lập Mật Mã & Quy Tắc Xác Thực Mã Token
- **Kiểm Soát Mật Mã Bất Đối Xứng:** Việc xác thực mã Token nội bộ yêu cầu bắt buộc áp dụng thuật toán mã hóa RS256, sử dụng các khóa công khai (Public Keys) tương ứng với từng môi trường cụ thể. Bộ lọc xác thực phải kiểm tra tính hợp lệ của chữ ký, thời hạn hết hạn (`exp`), và so khớp trường thông tin `tenant_id` được nhúng bên trong token.
- **Hạ Tầng Chống Tấn Công Phát Lại QR (Replay-Attack):** Bộ xử lý kiểm tra mã QR quét đầu vào phải ngay lập tức xác thực dấu thời gian (High-precision Timestamp) được mã hóa trong payload. Mọi lượt quét truyền tới máy chủ Backend vượt quá 30 giây kể từ thời điểm Client tạo mã thành công phải bị từ chối ngay lập tức (`400 Bad Request`) nhằm triệt tiêu rủi ro gian lận điểm danh bằng ảnh chụp màn hình hoặc video quay lại.
...
- **Coder Agent**: Đọc file `context.vi.md` tổng và dữ liệu ngữ cảnh của từng ngày. Hành xử nghiêm ngặt như một Kỹ sư trưởng Quarkus & Điện toán đám mây (tuyệt đối không sử dụng cú pháp của Spring Boot). Sinh mã nguồn nghiệp vụ hoàn chỉnh 100%, áp dụng cơ chế non-blocking theo tiêu chuẩn Mutiny và Jakarta EE, sau đó ghi trực tiếp vào đường dẫn được chỉ định trong khóa `target_component`.
...
- **DockerHub Agent:**
  * Vai trò: Kỹ sư quản lý kho lưu trữ container đa nền tảng (Cross-Platform Cloud Registry Engineer).
  * Nhiệm vụ: Chỉ kích hoạt thủ công khi có cờ `--release`. Chịu trách nhiệm bóc tách mã khóa từ khối `DOCKERHUB_SECRETS`, thực hiện đăng nhập bảo mật không tương tác (non-interactive login), biên dịch hình ảnh container cho cả Backend (GraalVM) và Frontend (Next.js), sau đó đẩy sản phẩm sạch lên kho lưu trữ Docker Hub dưới không gian tên (Namespace/Organization) chỉ định.
