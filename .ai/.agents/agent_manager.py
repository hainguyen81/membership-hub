...
class EnterpriseMultiAgentManager:
    ...
    def push_daily_git_branch(self, target_day):
        ...
        subprocess.run(["git", "add", target_day["target_component"]], capture_output=True)
        subprocess.run(["git", "add", target_day["test_component"]], capture_output=True)
        subprocess.run(["git", "add", target_day["doc_component"]], capture_output=True)
